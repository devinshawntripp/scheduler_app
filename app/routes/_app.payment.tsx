...
export default function Payment() {
    ...
    return (
        <div className="container mx-auto p-4">
            <h1 className="text-3xl font-bold mb-8 text-center text-primary">Upgrade Your Plan</h1>
            <p className="mb-8 text-center text-secondary">Current plan: <span className="font-semibold">{user.tier}</span></p>
                ...
                {plans.map((plan) => (
                    <div key={plan.name} className="card bg-base-100 shadow-xl">
                        <div className="card-body">
                            ...
                            <p className="text-3xl font-bold mb-6 text-accent">{plan.price}<span className="text-sm font-normal">/month</span></p>
                            ...
                            <div className="card-actions justify-end">
                                <Form method="post" className="w-full">
                                    ...
                                    <button
                                        type="submit"
                                        className={`btn btn-primary w-full ${user.tier === plan.name.toLowerCase() ? 'btn-disabled' : ''}`}
                                    >
                                        {user.tier === plan.name.toLowerCase() ? 'Current Plan' : 'Upgrade'}
                                    </button>
                                </Form>
                            </div>
                        </div>
